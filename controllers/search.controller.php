...
//		$language["gambits"]["more results"] => "s2",
	...
	if (!defined("AJAX_REQUEST")) {
		...
		$this->eso->addToBar("right", "<a href='" . makeLink("feed") . "' id='rss'><span class='button'><i class="icon-rss icon"></i></span></a>", 500);
		...
		if (!$this->eso->user) $this->eso->addToFooter("<a id='forgotPassword' href='" . makeLink("forgot-password") . "'>{$language["Forgot your password"]}</a>", 100);
		if ($this->eso->user) $this->eso->addToFooter("<a id='markAsRead' href='" . makeLink("?markAsRead") . "'>{$language["Mark all conversations as read"]}</a>", 200);
		if ($this->eso->user) $this->eso->addToFooter("<a id='startConversation' href='" . makeLink("conversation/new") . "'>{$language["Start a conversation"]}</a>", 300);
