<?php
// searchResults.inc.php
// Displays a table of search results using columns defined in the search controller.
...
{
    return "<a href='".makeLink($conversation["id"])."' style='margin:0' data-instant><img src='".$search->eso->getAvatar($conversation["startMemberId"],$conversation["avatarFormat"],"thumb")."' alt='' class='thumb'/></a>";
}
...
function columnConversation(&$search,$conversation)
{
    ...
    $html.="><a href='".makeLink($conversation["id"])."' data-instant>".highlight($conversation["title"],$_SESSION["highlight"])."</a></strong><br/>";
    ...
    if ($search->eso->user["name"] and $conversation["unread"]) $html .= "<small id='unreadPost'><a href='" . makeLink($conversation["id"], "?start=unread") . "'>{$language["Jump to unread"]}</a></small>";
    else $html .= "<small id='lastPost'><a href='" . makeLink($conversation["id"], "?start=last") . "'>{$language["Jump to last"]}</a></small>";
